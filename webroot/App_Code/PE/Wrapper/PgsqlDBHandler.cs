...
namespace HIT.PEditor.Wrapper 
{
    public class PgsqlDBHandler : PEditorFunctionsManager
    {
        ...
        public void SaveData()
        {
            ...
            try
            {
                ...
                using (NpgsqlConnection connection = new NpgsqlConnection(connectionString))
                {
                    ...
                    {
                        sqlBuilder.AppendFormat(@" {0} = '{1}', ", kvp.Key, kvp.Value.Replace("'","\\'"));
                    }
        ...
        public void ClearXMLDirectory(string location)
        {
            string[] files = Directory.GetFiles(location,"*.xml",SearchOption.TopDirectoryOnly);
            foreach (string fileName in files)
            {
                FileInfo file = new FileInfo(fileName);
                if (DateTime.Today.AddDays(-1).CompareTo(file.LastAccessTime) > 0)
                {
                    file.Delete();
                }
