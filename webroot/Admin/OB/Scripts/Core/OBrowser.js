...
var OBCore = function(showTabPanel) 
{
    ...
    this.FIXED_FIELD_SIZE_LIST = { '#': 25, ' ': 25, '': 25, 'Nr': 55, 'ADD': 25, 'EDIT': 30, 'PRINT':30, 'DELETE': 25, 'RECEIVE':30, 'ZOOM': 25, 'DETAIL': 30, 'BASKET': 25, 'THEME_COLOR':23, 'GROUP2FILTER':25,'EXPANDCOLLAPSE':25,'MULTISELECT':25};
    ...
    this.SQL_FIELD_TYPE = {};
    this.TOOLTIPS = { 'ADD': 'Add', 'EDIT': 'Edit', 'DELETE': 'Delete', 'ZOOM': 'Zoom', 'VIEW': 'View', 'PRINT':'Print', 'RECEIVE':'Receive','DETAIL':'Detail', 'GROUP2FILTER': 'Set current group as filter', 'BASKET': 'basket', 'collapse': 'Expand'   };
...
OBCore.prototype =
{
    ...
    ResetRowNumberColumnSize: function() {
        if (this.ACTIVE_GRID == "MAIN_GRID") {
            this.FIXED_FIELD_SIZE_LIST['#'] = (this.START_ROW + 10).toString().length * 10 + 5;
        } else {
            this.FIXED_FIELD_SIZE_LIST['#'] = (this.DETAIL_GRID_START_ROW + 10).toString().length * 10 + 5;
        }
    ...
    GetQuickSearchableFieldName: function() {
        var fieldName = "";
        if (this.SQL_GROUP_BY != 'NONE' && this.ACTIVE_GRID == "DETAIL_GRID") {
            if (!OBSettings.SQL_DETAIL_ORDER_BY) {
                fieldName = this.SQL_GROUP_BY;
            }
            else {
                fieldName = OBSettings.SQL_DETAIL_ORDER_BY;
            }
        }
    ...
    GetQuickSearchableFieldLabel: function() {
        ...
        if (this.SQL_GROUP_BY != 'NONE' && this.ACTIVE_GRID == "DETAIL_GRID") {
            if (!OBSettings.SQL_DETAIL_ORDER_BY) {
                ...
                fieldName = this.GetAlias(OBSettings.SQL_DETAIL_ORDER_BY);
            }
        }
        else if (this.SQL_GROUP_BY != 'NONE' && this.SQL_ORDER_BY) {
            fieldName = this.GetAlias(this.SQL_GROUP_BY);
    ...
    UpdateSortInfoInNavigation: function() {
        ...
        if (this.SQL_GROUP_BY != 'NONE' && this.ACTIVE_GRID == "DETAIL_GRID") {
            this.LoadOperatorList(OBSettings.SQL_DETAIL_ORDER_BY);
        }
        else {
