...
Ext.onReady(function() {
	...
	LoadReportList();
		...
		// RenderButton('footer','Goto Page',
		// function(){OBSettings.GotoPage();});
...
function InitReport() {
	...
	/* Added for filter implementation for detailGrid inside groupby grid */
	OBSettings.SQL_DETAIL_ORDER_BY = "";
	...
	if (OBSettings.SQL_GROUP_BY == "NONE") {
		...
		if (OBSettings.GIS_THEME_LAYER.toString() == 'true'
				|| OBSettings.GIS_THEME_LAYER.toString() == '1') {
	...
	if (ShowAddButton(OBSettings.REPORT_NAME) == false) {
		Ext.get("divAddRecord").setStyle('display', 'none');
	} else {
		Ext.get("divAddRecord").setStyle('display', 'block');
	}
}

function ShowAddButton(reportName) {
