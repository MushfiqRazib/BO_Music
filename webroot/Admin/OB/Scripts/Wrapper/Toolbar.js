function GetCurrentReportSettings() {
	...
	vis_fields += ',"qb_custom_fields":"' + OBSettings.QB_CUSTOM_FIELDS
			...
			+ '","sql_group_by":"' + OBSettings.SQL_GROUP_BY + '","page_size":'
			+ OBSettings.PAGE_SIZE + '}';
...
function SaveSelectedThemeColors(HeaderChekBoxName) {
	...
	if (currentGrid) {
		var groupCode = '';
		var colorCode = '';
		var selectedRecords = OBSettings.groupedGrid.getSelectionModel()
				.getSelections();
		// *** Add field data to the collection
		for (var i = 0; i < selectedRecords.length; i++) {
			colorCode += selectedRecords[i].data.THEME_COLOR
					.split('colorcode=')[1].split('>')[0].replace(/"/, '')
					.replace(/"/, '')
					+ ",";
		...
		var params = "{ REPORT_CODE:'" + OBSettings.REPORT_CODE + "', "
				+ "GROUP_BY:'" + OBSettings.SQL_GROUP_BY + "', "
				+ "GROUP_CODE:'" + groupCode + "', " + "COLOR_CODE:'"
				+ colorCode + "' }";
...
function OpenQueryBuilder() {
	var popup = OpenChild("./Query Builder/QueryBuilder.aspx", "QueryBuilder",
			true, 518, 395, "no", "no", false);
	popup.focus();
}
...
function OpenReportOption() {
	...
	if (OBSettings.COLOR_MODE == 1) {
		OpenChild('./Report/common_template.html?isGroupColoredID=1&fields='
						+ currentVisibleFields, 'PDFReportOptions', true, 330,
				150, 'no', 'no');
	} else {
		OpenChild('./Report/common_template.html?isGroupColoredID=null&fields='
						+ currentVisibleFields, 'PDFReportOptions', true, 330,
				150, 'no', 'no');
	}
}
