/* Project      - Bo01
 * Page         - supplyorder.aspx
 *
 * Developer    - Abdullah Al Mohammad
 *                Software Engineer,
 *                ERP Systems Ltd.
 * First updated - 19/07/2007
...
public partial class Admin_SupplyOrderDetail : System.Web.UI.Page
{    
...
//    DataTable articleTable = new DataTable();
    ...
    protected void Page_Load(object sender, EventArgs e)
    {
        if (!IsPostBack)
        {
            ...
            {
                LoadOrdersLine(false,"");//Load the mainTable first time
                InitializeOrderDate();//populate the order date field with current date
                LoadgrdArticle();//Load the article grid      
                
                    ((LinkButton)grdOrder.FooterRow.FindControl("lnkInsert")).Visible = false;//disable the insert button                        
                    ((LinkButton)grdOrder.Rows[0].FindControl("lnkDelete")).Visible = false;//disable the delete button
            ...
            else
            {
                ...
                {
                    Boeijenga.Common.Utils.LogWriter.Log("No such orders found");
                    throw new Exception("No such orders found");
                }
            }
...
//               "a.articlecode = s.articlecode and a.composer = p.composerid";
    ...
    /// Last updated - 19/07/2007
    /// </last>
    private void LoadOrdersLine(bool flag, string selectedcode)
    {
        if (!flag)//if the query is empty--generally when loading the table first time without orders
        {
        ...
        else //if the query is not empty
        {
            ...
            ((TextBox)grdOrder.FooterRow.FindControl("txtPrice")).Text = footerTable.Rows[0]["price"].ToString();
          //((DecimalControl)grdOrder.FooterRow.FindControl("txtVat")).Text = footerTable.Rows[0]["vat"].ToString();
            ((TextBox)grdOrder.FooterRow.FindControl("txtVat")).Text = footerTable.Rows[0]["vat"].ToString();
...
//                         order by a.title asc";
    ...
    protected void lnkSelect_Command(object sender, CommandEventArgs e)
    {
        ...
        foreach (GridViewRow row in grdArticle.Rows)
        {
            ...
            {
                flag = true;
               // string query = @"select a.articlecode, " +
               ...
               //     "a.articlecode='"+selectedCode+"' and a.composer = p.composerid";
        ...
        if (footerTable.Rows.Count > 0 && mainTable.Columns.Contains("articlecode"))
        {
            if (!AreadyExist())
            {
                ...
                //{
                    ...
                    //lnkSubmit.Visible = true;
                    //lnkSendOrder.Visible = true;
                    // footerTable.Rows.Remove(footerTable.Rows[0]);
    ...
    protected void lnkSubmit_Click(object sender, EventArgs e)
    {
        ...
        if (Page.IsValid)
        {
            ...
            //if (!EmptyOrderFields())//when the fields are not empty
            //{
               // string queryOrders = "";
               // string queryOrdersLine = "";
